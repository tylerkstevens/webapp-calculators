...
export function EconomicsSummary({ result, className = '' }: EconomicsSummaryProps) {
  ...
  return (
    <div className={`space-y-4 ${className}`}>
      {/* Header */}
      <h3 className="text-lg font-semibold text-surface-800">Economic Analysis</h3>
      ...
      <div className="bg-surface-50 rounded-lg p-4 space-y-3">
        <h4 className="text-sm font-semibold text-surface-700">Annual Breakdown</h4>

        <div className="space-y-2 text-sm">
          {/* Revenue */}
          <div className="flex justify-between">
            <span className="text-surface-600">Bitcoin Mining Revenue</span>
            <span className="font-medium text-green-600">
              +{formatCurrency(result.annualBtcRevenueUsd)}
            </span>
          </div>
          <div className="flex justify-between">
            <span className="text-surface-600">Heating Value (fuel avoided)</span>
            <span className="font-medium text-green-600">
              +{formatCurrency(result.annualHeatingValueUsd)}
            </span>
          </div>
          <div className="flex justify-between border-t border-surface-200 pt-2">
            <span className="text-surface-700 font-medium">Total Revenue</span>
          ...
          <div className="flex justify-between pt-2">
            <span className="text-surface-600">Electricity Cost</span>
            <span className="font-medium text-red-600">
              -{formatCurrency(result.annualElectricityCostUsd)}
            </span>
          </div>
          <div className="flex justify-between">
            <span className="text-surface-600">Pool Fees</span>
            <span className="font-medium text-red-600">
              -{formatCurrency(result.annualPoolFeeUsd)}
            </span>
          </div>
          <div className="flex justify-between border-t border-surface-200 pt-2">
            <span className="text-surface-700 font-medium">Total Costs</span>
          ...
          {/* Net */}
          <div className="flex justify-between border-t-2 border-surface-300 pt-2">
            <span className="text-surface-800 font-semibold">Net Annual Profit</span>
            <span
              className={`font-bold ${
                result.annualNetProfitUsd > 0 ? 'text-green-700' : 'text-red-700'
              }`}
            >
      ...
      <div className="bg-white rounded-lg border border-surface-200 p-4">
        <h4 className="text-sm font-semibold text-surface-700 mb-3">Revenue Split</h4>
